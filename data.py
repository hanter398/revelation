...
def get_team_stats():
    url = f'https://gol.gg/teams/list/season-S14/split-ALL/tournament-ALL/'
    data = pd.read_html(requests.get(url, headers={'User-agent': 'your bot 0.1'}).text)[3]
    data.to_csv("./teamstats.csv")

def get_player_stats():
    url = f'https://gol.gg/players/list/season-S14/split-ALL/tournament-ALL/'
    data = pd.read_html(requests.get(url, headers={'User-agent': 'your bot 0.1'}).text)[4]
    data.to_csv("./playerstats.csv")
